		...
		for contact in driver.get_unread(include_me=True, include_notifications=True):
			for message in contact.messages:
				...
				if message.type == 'chat':
					...
					f=open("chat_" + message.chat_id + ".chat.log","a+", encoding="utf-8")
					...
					f=open("safechat_" + message.chat_id + ".chat.log","a+")
					...
					if ( (message.chat_id == masters_number+"@c.us") and (message.content[0:1]=="#") ) :
					...
					f=open("chat_" + message.chat_id + ".chat.log","a+")
					...
					f.write("[ {sender} | {timestamp} ] sent media chat_{id}\{filename} with caption '{caption}'\n".format(sender=message.sender.get_safe_name(), timestamp=message.timestamp, id=message.chat_id, filename=message.filename, caption=msg_caption))
					f.close()
					# f=open("safechat_" + message.chat_id['_serialized'] + ".chat.log","a+")
					f=open("safechat_" + message.chat_id + ".chat.log","a+")
					# f.write("[ {sender} | {timestamp} ] sent media chat_{id}\{filename} with caption '{caption}'\n".format(sender=message.sender.get_safe_name(), timestamp=message.timestamp, id=message.chat_id['_serialized'], filename=message.filename, caption=msg_caption))
					...
					if not os.path.exists('chat_{id}'.format(id=message.chat_id)):
						os.makedirs('chat_{id}'.format(id=message.chat_id))
					message.save_media('chat_{id}'.format(id=message.chat_id))
				else:
					print('-- Other')
